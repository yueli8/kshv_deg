...
#wt_vs_mock
cts<-read.table("wt_vs_mock.txt",head=TRUE) 
#setup DESeqDataSetFromMatrix
cts_round<-round(cts)
countData<-cts_round
condition <- factor(c(rep("wt",11),rep("mock",9)), levels = c("wt","mock"))
coldata<-data.frame(row.names=colnames(countData),condition)
condition
coldata
dds<-DESeqDataSetFromMatrix(countData=countData, colData=coldata, design=~condition)
...
dds <- dds[keep,]
dim(dds)
dds$condition <- factor(dds$condition, levels = c("wt","mock"))
dds$condition <- relevel(dds$condition, ref = "mock")
dds$condition <- droplevels(dds$condition)
dds <- estimateSizeFactors(dds)
dds_norm<-counts(dds,normalized=TRUE)
head(dds_norm)
write.csv(dds_norm,"wt_vs_mock_norm.csv")
dds2<-DESeq(dds)
resultsNames(dds2)
res <- results(dds2)
res <- results(dds2, contrast=c("condition","wt","mock"))
resultsNames(dds2)
write.csv(res,"wt_vs_mock_deg.csv")
...
#volcano plot wt_vs_mock_DEG
res <- read.csv("wt_vs_mock_deg.csv", header=TRUE)
head(res)
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(-6,6)))
# Add colored points: red if pvalue<0.05, orange of log2FC>1, green if both)
with(subset(res, pvalue<.05 ), points(log2FoldChange, -log10(pvalue), pch=20, col="red"))
